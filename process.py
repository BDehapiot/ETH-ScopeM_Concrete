...
stack_names = ["D1_ICONX_DoS", "D11_ICONX_DoS", "D12_ICONX_DoS", "H9_ICONX_DoS"]
...
def get_3Dmasks(
    ...
    img_norm = median(img_norm, footprint=disk(2)) # parameter
    img_norm *= mtx_mask
...
distances = np.linalg.norm(mtch_coords_ref - mtch_coords_reg, axis=1)
...
# mtch_dist_abs = np.abs(mtch_dist_ref - mtch_dist_reg)
# for i in range(mtch_dist_abs.shape[0]):
#     tmp_abs = mtch_dist_abs.copy()
#     tmp_abs = np.delete(tmp_abs, i, axis=0)
#     tmp_abs = np.delete(tmp_abs, i, axis=1) 
#     mtch_dist_scr.append(np.mean(tmp_abs))
# mtch_dist_scr = np.stack(mtch_dist_scr) 
# mtch_dist_scr /= np.median(mtch_dist_scr)
# outliers = np.where(mtch_dist_scr < 0.9)[0]
